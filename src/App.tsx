    ...
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
      try {
        ...
        recognition.onresult = async (event: any) => {
          ...
          // First try power shortcuts
          const shortcutHandled = await powerUserSystem.processVoiceTrigger(transcript)
          if (shortcutHandled) {
            toast.success(`⚡ Power shortcut executed: "${transcript}"`)
            setVoiceListening(false)
            return
          }

          // Then try enhanced AI commands
    ...
    const initializeEnhancedSystem = async () => {
      ...
      try {
        ...
        console.log(`⚡ ${shortcuts.length} power shortcuts available`)
          ...
          testAIAccess: async (agent: any, action: string, params: any) => {
...
Provide concise, actionable advice focused on field service work.`
  ...
  const ModeToggle = () => (
    <div className="flex items-center gap-2">
      <button
        ...
        className={`px-3 py-1 text-xs font-medium rounded-full transition-all ${appMode === 'tech'
      ...
      <button
        ...
        className={`px-2 py-1 text-xs rounded-full transition-all ${voiceListening
  ...
  // EXCEPTION: Jon's Personal Build gets full access even on mobile
  if ((isMobile && !isPersonalBuild) || isFieldTechOnly(currentUser)) {
    return (
      <MobileLayout
        activeTab={activeTab}
  ...
  return (
    <div className="min-h-screen bg-background" data-theme="contrast">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1">
        <TabsList className="flex w-full overflow-x-auto pb-2 no-scrollbar whitespace-nowrap">
